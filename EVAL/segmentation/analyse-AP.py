...
def validate_args(args):
    ...
    if validate_mandatory_args(args.gtFolder, '-gt/--gt-folder', errors):
        ...
        errors.pop()
        args.gtFolder = os.path.join(current_path, 'groundtruths-txt')
    ...
    if args.gtCoordType == CoordinatesType.Relative:  # Image size is required
        args.imgSize = validate_image_size(args.imgSize, '--img-size', '--gt-coords', errors)
    #if args.detCoordType == CoordinatesType.Relative:  # Image size is required
    #    args.imgSize = validate_image_size(args.imgSize, '--img-size', '--det-coords', errors)
    # Detection folder
    if validate_mandatory_args(args.detFolder, '-det/--det-folder', errors):
        ...
        errors.pop()
        args.detFolder = os.path.join(current_path, 'detections-txt')
...
def main():
    ...
    print("Number of BBox in the GT:", all_bounding_boxes.count())
    ...
    print("--------------------------------")
    print("Classes:", all_classes)
    print("Number of BBox in the detection:", all_bounding_boxes.count())
